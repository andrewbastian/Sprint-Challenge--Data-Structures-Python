...
given order. It should not return any `None` values in the list even if they
are present in the ring buffer.
...
class RingBuffer:
    ...
    def append(self, item):
        if len(self.queue) < self.capacity:
            ...
            self.queue[self.size % (self.capacity)] = item
            # Modulo used to find remainder and oldest value
        self.size += 1
